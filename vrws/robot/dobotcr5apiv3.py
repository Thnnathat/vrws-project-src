...
from .aliengripper.aliengripperapi import AlienGripper
...
class DobotCR5ApiV3:
    def __init__(self, ip: str="192.168.5.1") -> None:
        ...
        self.gripper: AlienGripper = AlienGripper("192.168.1.10", "81", "ws")
        
        self.speed = 100
        dashboard.SpeedL(self.speed)
    ...
    def RunToPoint(self, point_list: list[float], coordinateMode: int, condition: int = False):
        ...
        while True:
            ...
            if p2id[0] == 0: # ไม่มี error (ถ้ามี error จะทำการส่งคำสั่งใหม่เรื่อยๆ)
                self.WaitArrive(point_list, condition)
                break
    ...
    def WaitArrive(self, point_list, condition):
        while True:
            is_arrive = True
            ...
            if current_actual is not None:
                for index in range(4):
                    if (abs(current_actual[index] - point_list[index]) > 1):
                        is_arrive = False
                if condition:
                    is_arrive = self.WaitCondition(point_list)
                if is_arrive:
    ...
    def WaitCondition(self, point_list):
        is_arrive = False
        data = self.gripper.getData()
        presure = data["sensors"]["pressure"]
        sw = data["sensors"]["switch"]
        print(data)
        nearObject = [abs(current_actual[i] - point_list[i]) for i in range(4)]
        if nearObject[2] < 200:
            if ~self.is_holding:
                self.gripper.hold()
                self.is_holding = True
            self.dashboard.SpeedFactor(2)
            if (presure <= 5000) or sw == 0:
                self.dashboard.ResetRobot()
                self.dashboard.RobotMode()
